...
CSAK SIMA APOSZTROF JÓ, DUPLA NEM
```js
select title
from movies_and_tv
...
```
mindig a clustering key (itt foglalkozas) szerint rakja sorba
...
insert into szemely (nev, szulev, foglalkozas) values ('Otto', 2005, 'lakatos222');
insert into szemely (nev, szulev, foglalkozas) values ('harmadik ember', 2008, 'lakatos666');
...
desc szemely; //itt látszanak az indexei is a táblának
```
...
select nev
from szemely
where foglalkozas = 'pincer' OR foglalkozas = 'lakatos'; // ez nem jó!!!
...
where nev = 'Otto' AND szulev = 2005 AND foglalkozas = 'lakatos222';
